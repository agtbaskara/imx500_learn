...
# Updated transform with augmentations to simulate real-world camera conditions
transform = transforms.Compose([
    ...
    # Add random brightness variation to simulate shadows and lighting changes
    transforms.ColorJitter(brightness=0.3, contrast=0.3),
    
    # Add slight gaussian noise to simulate camera sensor noise
    transforms.Lambda(lambda x: x + torch.randn_like(x) * 0.05),
    
    # Random affine transformation for perspective and rotation variation
    transforms.RandomAffine(
        ...
        shear=5  # Random shearing up to 5 degrees
    ),
    
    # Random perspective transform for realistic camera angle variations
    transforms.RandomPerspective(distortion_scale=0.2, p=0.5),
    
    # Occasional slightly blurry images to simulate focus issues
    transforms.RandomApply([transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.0))], p=0.3),
    
    # Ensure values stay in valid range [0,1] after all transformations
    transforms.Lambda(lambda x: torch.clamp(x, 0, 1))
])

# For validation/testing, you might want a separate transform without the random augmentations
...
testset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform_val)
